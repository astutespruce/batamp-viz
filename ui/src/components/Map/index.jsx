...
const Map = ({
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      // TODO: optimization: don't set filter if full set of hexes
      const h3LayerFilterExprs = Object.fromEntries(
        H3_COLS.map((col) => [
          col,
          [
            'in',
            ['id'],
            ['literal', Object.keys(h3Totals[col]).map((v) => parseInt(v, 10))],
          ],
        ])
      )

      console.log('filter exprs', h3LayerFilterExprs)
      ...
      H3_COLS.forEach((col) => {
        ...
        map.on('mousemove', fillLayerId, ({ features: [feature], lngLat }) => {
          ...
          if (hoverFeature !== hoverFeatureRef.current) {
            ...
            setHexHighlight(map, hoverFeatureRef.current, false)

            hoverFeatureRef.current = hoverFeature
            setHexHighlight(map, hoverFeatureRef.current, true)
          }
        })
  ...
  // FIXME: need to prevent this from firing when style is reloading
  useEffect(() => {
    ...
    if (dequal(h3Totals, h3TotalsRef.current)) return

    console.log('handle updated hex totals')
